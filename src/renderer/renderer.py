...
from ..domain import ReviewResult, Issue, Severity
...
class Renderer:
    ...
    def to_github_summary(self, result: ReviewResult, risk_score: int) -> str:
        ...
        """
        icon = "✅" if result.decision == "PASS" else ("⚠️" if result.decision == "WARN" else "🚫")
        ...
        if result.issues:
            md += "### Top Issues\n"
            # Show top 5 blockers/important
            top_issues = [i for i in result.issues if i.severity in (Severity.BLOCKER, Severity.IMPORTANT)][:5]
            if not top_issues:
                top_issues = result.issues[:3]
                
            for issue in top_issues:
                link = f"{issue.path}#L{issue.line_start}"
                md += f"- [{issue.path}]({link}): **{issue.title}** ({issue.severity})\n"
