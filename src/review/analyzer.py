...
from ..domain import ChangedFile, Issue, Evidence, EvidenceType
...
class ReviewAnalyzer:
    ...
    def _extract_diff_excerpt(self, file: ChangedFile, line_start: int, line_end: int) -> str:
        """
    ...
    def triage(self, filter_result: FilterResult, pr_meta: dict) -> Dict:
        """
...
"""
        files_summary = "\n".join([f"{f.path} (+{f.additions}/-{f.deletions})" for f in filter_result.files_to_review])
...
"""
        ...
        try:
            response = self.llm.get_completion(system_prompt, user_prompt, response_format={"type": "json_object"})
    ...
    def review_file(self, file: ChangedFile, docs_evidence: List[Evidence]) -> List[Issue]:
        """
...
"""
        ...
        try:
            # We trust LLM output to not leak secrets back, but we should be careful.
            # Ideally we'd redact the response too, but that might mask suggestions.
            # For now, let's proceed with caution.
            response = self.llm.get_completion(system_prompt, safe_user_prompt, response_format={"type": "json_object"})
            
            # Safe parsing with repair attempt
            try:
                data = SafeJSONParser.parse(response)
            except Exception:
                # One retry with repair prompt? Or just fail gracefully for this file
            ...
            for item in data.get("issues", []):
                ...
                evidence = self._build_issue_evidence(file, docs_evidence, line_start, line_end)

                # Map back to domain Issue
                issues.append(Issue(
                    ...
                    evidence=evidence,
                    # We will fill position and fingerprint later in Policy/Manager.
                ))
            return issues
        except Exception as e:
