...
from src.domain import ChangedFile, DiffHunk
...
class TestReviewFlow(unittest.TestCase):
    def test_filter_logic(self):
        files = [
            ChangedFile(
                path="src/main.py", status="modified", additions=10, deletions=2
            ),
            ChangedFile(
                path="package-lock.json", status="modified", additions=100, deletions=0
            ),
            ChangedFile(
                path="src/auth/login.py", status="added", additions=50, deletions=0
            ),
        ]
    ...
    def test_review_adds_diff_evidence_and_passes_policy(self, MockLLM):
        ...
        if evidence is None:
            self.fail("Expected evidence to be populated for reviewed issue")
        self.assertEqual(evidence.type.value, "DIFF")
